...
Which maps all positive inputs to themselves while negative inputs are mapped to zero. The ReLU activation function introduces non-linearity to the model and mitigates the common vanishing and exploding gradient problem that occurs in deep networks. On the other hand, for the final output layer, the softmax activation function is commonly used. It maps the input to a vector whose values sum up to 1, which is great for a probabilistic interpretation.
...
During the training process, optimization strategies such as stochastic gradient descent (SGD) are used. The gradient of the cost function is computed with respect to the weights, which is just for adjusting the weights in the direction that minimizes the cost:
